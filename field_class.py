...
class Field:
    ...
    def grow(self,light, food, water):
        #grow the crop(light and water are available in the same amount)
        if len(self._crops) > 0:
            for crop in self._crops:
                crop.grow(light, water)
        if len(self._animals) > 0:
            #grow the animal(water available to all animals in same amount)
            #food is the total that must be shared
            food_required = 0
            #get the total food for animals in field
            for animal in self._animals:
                needs = animal.needs()
                food_required += needs["food need"]
            #share remaining food
...
def display_main_menu():
    ...
    print()
    print("5.Grow a field manually over 1 day")
